...
export async function generateMetadata({
  ...
  return {
    ...
    description:
      showroom.description || `Explore ${showroom.name} in ${showroom.city}.`,
  };
}

export default async function ShowroomProfilePage({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
  ...
  return (
    <main className="w-full bg-background min-h-screen text-[#E8E8E8] selection:bg-[#FDFCFB] selection:text-[#0A0A0A] pb-32">
      ...
      <div className="fixed top-0 left-0 w-full z-40 p-6 lg:p-12 pointer-events-none mt-16 lg:mt-0">
        <Link
          href="/showrooms"
          className="pointer-events-auto inline-flex items-center gap-3 bg-[#050505]/80 backdrop-blur-md border border-[rgba(255,255,255,0.05)] px-6 py-3 rounded-full hover:bg-[#111] transition-all group shadow-2xl"
        >
          <ArrowLeft className="w-4 h-4 text-[#8D8D8D] group-hover:text-[#FDFCFB] group-hover:-translate-x-1 transition-transform" />
          <span className="font-heading uppercase text-[10px] tracking-[0.2em] font-semibold text-[#8D8D8D] group-hover:text-[#FDFCFB]">
            Showrooms Network
          </span>
        </Link>
      </div>
              ...
              <div className="flex flex-wrap items-center gap-4 mb-6">
                ...
                {verification && (
                  <div className="flex items-center gap-1.5 bg-[#FDFCFB] px-3 py-1.5">
                    ...
                    <span className="font-heading text-[10px] font-bold uppercase tracking-[0.2em] text-[#0A0A0A]">
                      Verified Partner
                    </span>
                  </div>
                )}
                {hours && (
                  <div className="flex items-center gap-1.5 bg-[#050505]/80 backdrop-blur-md border border-[rgba(255,255,255,0.1)] px-3 py-1.5">
                    ...
                    <span className="font-mono text-[9px] uppercase tracking-widest text-[#8D8D8D]">
                      {hours}
                    </span>
                  </div>
                )}
          ...
          <div className="lg:col-span-8 flex flex-col gap-12">
            ...
            {history && (
              <div className="p-8 bg-[#050505] border border-[rgba(255,255,255,0.02)]">
                <h4 className="font-heading text-[9px] uppercase tracking-[0.3em] text-[#4A4A4A] mb-4">
                  Heritage
                </h4>
        ...
        <section className="mb-32 border-b border-[rgba(255,255,255,0.05)] pb-32">
          <div className="flex items-center justify-between mb-16">
            <div>
              ...
              <h3 className="font-display italic text-4xl text-[#FDFCFB]">
                Resident Creators
              </h3>
            </div>
          ...
          {creators.length === 0 ? (
            <div className="w-full py-16 border border-[rgba(255,255,255,0.02)] bg-[#050505] text-center">
              <p className="font-heading text-xs uppercase tracking-widest text-[#4A4A4A]">
                No creators currently affiliated.
              </p>
            </div>
          ) : (
              ...
              {creators.map(
                (c: {
                  ...
                  <Link
                    ...
                    className="group p-6 bg-[#050505] border border-[rgba(255,255,255,0.02)] hover:border-[rgba(255,255,255,0.15)] transition-colors"
                  >
                    <div className="flex flex-col items-center text-center">
                      <div className="w-20 h-20 rounded-full bg-[#111] overflow-hidden relative mb-4">
                        {c.user.avatar ? (
                          <Image
                            src={c.user.avatar}
                            alt={c.user.displayName || c.user.username}
                            fill
                            className="object-cover"
                          />
                        ) : (
                      ...
                      <h4 className="font-heading text-sm uppercase tracking-wider text-[#E8E8E8] group-hover:text-[#FDFCFB]">
                        {c.user.displayName || c.user.username}
                      </h4>
                      <p className="font-mono text-[9px] text-[#4A4A4A] mt-2 uppercase tracking-widest">
                        @{c.user.username}
                      </p>
                    </div>
                  </Link>
        ...
        <section>
          ...
          {products.length === 0 ? (
            <div className="w-full py-24 border border-[rgba(255,255,255,0.02)] bg-[#050505] text-center">
              <p className="font-heading text-xs uppercase tracking-widest text-[#4A4A4A]">
                This showroom has not listed any physical inventory yet.
              </p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {products.map(
                (p: {
                  ...
                  <div
                    key={p.id}
                    className="group relative bg-[#050505] border border-[rgba(255,255,255,0.02)]"
                  >
                    ...
                    <div className="p-6">
                      <h4 className="font-heading text-lg uppercase tracking-wider text-[#E8E8E8]">
                        {p.product.name}
                      </h4>
                      <p className="font-mono text-[10px] text-[#8D8D8D] mt-2">
                        ${p.product.price}
                      </p>
                    </div>
                  </div>
