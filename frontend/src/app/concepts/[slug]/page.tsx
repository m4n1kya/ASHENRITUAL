...
export async function generateMetadata({
  ...
  return {
    ...
    description:
      concept.description || `Explore ${concept.title} on ASHENRITUAL.`,
  };
}

export default async function ConceptDetailsPage({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
    ...
    <main className="w-full bg-background min-h-screen text-[#E8E8E8] selection:bg-[#FDFCFB] selection:text-[#0A0A0A] pb-32">
      ...
      <div className="fixed top-0 left-0 w-full z-40 p-6 lg:p-12 pointer-events-none flex justify-between items-start mt-16 lg:mt-0">
        <Link
          ...
          className="pointer-events-auto flex items-center gap-3 bg-[#050505]/80 backdrop-blur-md border border-[rgba(255,255,255,0.05)] px-6 py-3 rounded-full hover:bg-[#111] transition-all group shadow-2xl"
        >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-16">
            ...
            {gallery && gallery.length > 0 && (
              <div className="flex flex-col gap-8 mt-16">
                {gallery.map((img: string, i: number) => (
                  <div
                    key={i}
                    className="relative w-full bg-[#050505] overflow-hidden border border-[rgba(255,255,255,0.02)]"
                  >
                    <Image
                      src={img}
          ...
          <div className="lg:col-span-4 flex flex-col gap-12">
            ...
            <div className="bg-[#050505] border border-[rgba(255,255,255,0.05)] p-8 relative group">
              ...
              />
              <h3 className="font-heading text-[9px] uppercase tracking-[0.3em] text-[#4A4A4A] mb-6">
                Designed By
              </h3>
              <div className="flex items-center gap-4 mb-6">
                ...
                <div>
                  <h4 className="font-heading text-sm uppercase tracking-wider text-[#FDFCFB] flex items-center gap-2">
                    ...
                    {creator?.verified && (
                      <CheckCircle className="w-3.5 h-3.5 text-[#FDFCFB]" />
                    )}
                  </h4>
                  <p className="font-mono text-[9px] text-[#8D8D8D] uppercase tracking-widest mt-1">
                    @{user?.username}
                  </p>
                </div>
              </div>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {materials.map((m: string, i: number) => (
                      <span
                        key={i}
                        className="px-3 py-1.5 bg-[#050505] border border-[rgba(255,255,255,0.05)] text-[#A8A8A8] text-[10px] uppercase tracking-widest font-mono"
                      >
                        {m}
                      </span>
                    ))}
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {tags.map((t: string, i: number) => (
                      <span
                        key={i}
                        className="px-3 py-1.5 text-[#8D8D8D] text-[10px] uppercase tracking-widest font-mono hover:text-[#FDFCFB] transition-colors cursor-pointer"
                      >
                        #{t}
                      </span>
                    ))}
