...
export async function generateMetadata({
  ...
  return {
    ...
    description:
      profile.bio ||
      `Explore the concepts and collections of ${profile.displayName || profile.username}.`,
  };
}
          ...
          <div className="flex flex-col gap-6">
            ...
            <div>
              ...
              <p className="font-mono text-[11px] uppercase tracking-widest text-[#8D8D8D] flex items-center gap-2">
                ...
                <span className="mx-2 opacity-30">|</span>@{profile.username}
              </p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-16">
          ...
          <div className="lg:col-span-3 flex flex-col gap-12">
            <section>
              ...
              <p className="font-sans text-sm text-[#A8A8A8] leading-relaxed">
                {profile.bio ||
                  "This designer prefers their work to speak for itself."}
              </p>
            </section>

            <section>
              <h3 className="font-heading text-[10px] uppercase tracking-[0.3em] text-[#4A4A4A] mb-4">
                Focus
              </h3>
              <div className="flex flex-wrap gap-2">
                {(profile.creatorProfile?.specialization
                  ? profile.creatorProfile.specialization.split(",")
                  : ["Design", "Art"]
                ).map((spec, i) => (
                  <span
                    key={i}
                    className="px-3 py-1 bg-[#111] text-[#8D8D8D] text-[10px] uppercase tracking-widest font-mono"
                  >
                    {spec.trim()}
                  </span>
                ))}
                ...
                <div className="flex flex-col gap-3">
                  {showrooms.map(
                    (affiliation: {
                      ...
                      <Link
                        key={affiliation.id}
                        href={`/showrooms/${affiliation.showroom.slug}`}
                        className="group flex items-center gap-3 p-3 bg-[#050505] border border-[rgba(255,255,255,0.05)] hover:border-[rgba(255,255,255,0.15)] transition-colors"
                      >
                        <div className="w-8 h-8 bg-[#111] rounded-full overflow-hidden relative">
                          {affiliation.showroom.logo ? (
                            <Image
                              src={affiliation.showroom.logo}
                              alt={affiliation.showroom.name}
                              fill
                              className="object-cover"
                            />
                          ) : (
                        ...
                        <div className="flex-1">
                          <p className="font-heading text-xs uppercase tracking-wider text-[#E8E8E8] group-hover:text-[#FDFCFB]">
                            {affiliation.showroom.name}
                          </p>
                          <p className="font-mono text-[9px] text-[#4A4A4A]">
                            {affiliation.showroom.city || "Global"}
                          </p>
                        </div>
            ...
            <section>
              ...
              <div className="flex flex-col gap-3">
                {[1, 2].map((i) => (
                  <div
                    key={i}
                    className="flex items-center gap-3 opacity-60 hover:opacity-100 cursor-not-allowed transition-opacity"
                  >
                    <div className="w-8 h-8 rounded-full bg-[#111]" />
                    <div>
                      <p className="font-heading text-xs uppercase tracking-wider text-[#8D8D8D]">
                        Designer {i}
                      </p>
                      <p className="font-mono text-[9px] text-[#4A4A4A]">
                        Similar Philosophy
          ...
          <div className="lg:col-span-9">
            <div className="flex items-center justify-between mb-8 border-b border-[rgba(255,255,255,0.05)] pb-4">
              <h2 className="font-display italic text-3xl text-[#FDFCFB]">
                The Exhibition
              </h2>
              <div className="flex items-center gap-2 text-[#8D8D8D]">
                <Grid className="w-4 h-4" />
                <span className="font-mono text-[10px] uppercase tracking-widest">
                  {concepts.length} Pieces
                </span>
              </div>
            </div>
            ...
            {concepts.length === 0 ? (
              <div className="w-full py-24 flex flex-col items-center justify-center border border-[rgba(255,255,255,0.02)] bg-[#050505]">
                ...
                <p className="text-[#4A4A4A] text-xs max-w-sm text-center">
                  This designer hasn&apos;t published any concepts yet.
                </p>
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {concepts.map((concept: Concept) => (
                  <Link
                    ...
                    className="group relative block overflow-hidden bg-[#050505] aspect-[3/4]"
                  >
                    <Image
                      src={concept.image}
                      alt={concept.title}
