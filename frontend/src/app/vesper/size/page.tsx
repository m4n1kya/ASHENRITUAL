...
export default function SizeIntelligencePage() {
  ...
  const [step, setStep] = useState<
    "INPUT" | "SCANNING" | "ANALYZING" | "REPORT"
  >("INPUT");
  ...
  const startCamera = async () => {
    try {
      ...
      alert(
        "Camera access is required for Architectural Body Analysis. Please ensure no other app (like Zoom) is using it.",
      );
    }
  };
      ...
      if (video.currentTime !== lastVideoTime) {
        ...
        if (results.landmarks && results.landmarks.length > 0) {
          ...
          if (lShoulder && rShoulder) {
            ...
            const estimatedShouldersCm = Math.round(
              pixelDist * parseInt(heightCm) * 1.5,
            );

            setEstimates((prev) => ({
              ...prev,
              shoulderWidthCm: estimatedShouldersCm,
            }));
  ...
  const finalizeAnalysis = async () => {
    ...
    const finalMeasurements: BodyMeasurements = {
      ...
      chestCircumferenceCm: estimates.shoulderWidthCm
        ? estimates.shoulderWidthCm * 2.2
        : 96,
      waistCircumferenceCm: estimates.shoulderWidthCm
        ? estimates.shoulderWidthCm * 1.8
        : 80,
      sleeveLengthCm: 64,
  ...
  return (
    <div className="h-full w-full overflow-y-auto px-6 md:px-12 pt-32 pb-12 hide-scrollbar relative">
            ...
            <AnimatePresence mode="wait">
                ...
                >
                  ...
                  <div className="grid grid-cols-2 gap-6 w-full max-w-sm mb-12">
                    <div className="flex flex-col text-left">
                      <label className="text-[9px] uppercase tracking-widest text-[#8D8D8D] mb-2">
                        Height (cm)
                      </label>
                      <input
                        type="number"
                        ...
                        className="bg-transparent border-b border-[#333] text-[#FDFCFB] px-0 py-2 focus:ring-0 focus:border-[#FDFCFB] transition-colors"
                      />
                    </div>
                    <div className="flex flex-col text-left">
                      <label className="text-[9px] uppercase tracking-widest text-[#8D8D8D] mb-2">
                  ...
                  <p className="mt-6 text-[9px] uppercase tracking-widest text-[#4A4A4A]">
                    All body analysis is processed securely locally. Images are
              ...
              {(step === "SCANNING" || step === "ANALYZING") && (
                ...
                >
                  ...
                  <canvas
                    ref={canvasRef}
                    className="absolute inset-0 w-full h-full object-cover mix-blend-screen -scale-x-100"
                  />
            ...
            <AnimatePresence mode="wait">
              {step === "INPUT" && (
                <motion.div
                  ...
                  exit={{ opacity: 0, x: 20 }}
                >
                  <h4 className="font-heading text-sm uppercase tracking-widest text-[#FDFCFB] mb-4">
                    The Process
                  </h4>
                  <p className="text-xs text-[#8D8D8D] leading-relaxed mb-6">
                    Our Size Intelligence engine combines MediaPipe
                    architectural landmarks with advanced proportional
                    algorithms to map your unique silhouette without requiring
                    manual tape measurements.
                  </p>
                  <div className="space-y-4 text-[10px] uppercase tracking-widest text-[#4A4A4A]">
                    <div className="flex items-center gap-3">
                      <div className="w-1 h-1 bg-[#4A4A4A]" /> 1. Establish
                      Baseline
                    </div>
                    <div className="flex items-center gap-3">
                      <div className="w-1 h-1 bg-[#4A4A4A]" /> 2. Align
                      Architecture
                    </div>
                    <div className="flex items-center gap-3">
                      <div className="w-1 h-1 bg-[#4A4A4A]" /> 3. Interpreting
                      Proportions
                    </div>
                    <div className="flex items-center gap-3">
                      <div className="w-1 h-1 bg-[#4A4A4A]" /> 4. Tailoring
                      Recommendation
                    </div>
                  </div>
                </motion.div>
                ...
                >
                  ...
                  <div className="space-y-6">
                    <div>
                      ...
                      <p className="font-mono text-2xl text-[#E8E8E8]">
                        {estimates.shoulderWidthCm
                          ? `${estimates.shoulderWidthCm} cm`
                          : "---"}
                      </p>
                    </div>
                    <div>
              ...
              {step === "ANALYZING" && (
                ...
                >
                  ...
                  <p className="font-heading text-[11px] uppercase tracking-[0.3em] text-[#8D8D8D] mb-2">
                    Interpreting Proportions
                  </p>
                  <p className="text-[9px] uppercase tracking-widest text-[#4A4A4A] animate-pulse">
                    Running Architectural Reasoning...
                  </p>
                </motion.div>
              )}
              ...
              {step === "REPORT" && profile && (
                <motion.div
                  ...
                  className="bg-[#111] p-8 border border-[#202020] rounded-sm"
                >
                  ...
                  <div className="grid grid-cols-2 gap-4 mb-6">
                    <div>
                      <p className="text-[9px] uppercase tracking-widest text-[#8D8D8D]">
                        Type
                      </p>
                  ...
                  <p className="text-xs text-[#8D8D8D] leading-relaxed italic border-l-2 border-[#333] pl-4 mb-8">
                    &quot;{profile.measurements.shoulderWidthCm}cm shoulders
                    with a {profile.measurements.chestCircumferenceCm}cm
                    chest.&quot;
                    <br />
                    <br />
                    This profile guarantees structural precision across the
                    ASHENRITUAL permanent collection.
                  </p>
