...
import { useSizeStore, BodyMeasurements, BodyProfile } from '@/store/size.store';
  ...
  if (!profile) {
    return (
      <div className="h-full w-full flex items-center justify-center bg-background texture-grain px-12">
        <p className="text-[11px] uppercase tracking-widest text-[#4A4A4A]">No architectural profile established.</p>
      </div>
    );
  }
  ...
  const renderField = (key: keyof BodyMeasurements, label: string) => {
    ...
    return (
      <div className="flex flex-col border-b border-[#202020] py-6 group">
        <label className="text-[9px] uppercase tracking-[0.3em] text-[#8D8D8D] mb-3">{label}</label>
        {isEditing ? (
          ...
          <p className="text-xl font-mono text-[#E8E8E8] group-hover:text-[#FDFCFB] transition-colors">{value} <span className="text-[#8D8D8D] text-sm">cm</span></p>
        )}
      </div>
    );
  };
    ...
    <div className="h-full w-full px-12 py-12 overflow-y-auto hide-scrollbar relative">
      <div className="max-w-4xl mx-auto h-full flex flex-col">
        ...
        <header className="mb-16 flex items-end justify-between border-b border-[#202020] pb-8">
          ...
          <AnimatePresence mode="wait">
            {!isEditing ? (
              ...
              >
                <button onClick={handleCancel} className="text-[10px] uppercase tracking-widest text-[#8D8D8D] hover:text-[#E8E8E8] flex items-center gap-2 transition-colors">
                  <X className="w-3 h-3" /> Cancel
                </button>
                <button onClick={handleSave} className="text-[10px] uppercase tracking-widest text-[#FDFCFB] flex items-center gap-2 bg-[#1A1A1A] px-4 py-2 rounded-sm hover:bg-[#202020] transition-colors">
        ...
        <div className="mt-16 bg-[#0A0A0A] border border-[#202020] p-8 flex items-center justify-between">
          <div>
            <p className="text-[10px] uppercase tracking-widest text-[#8D8D8D] mb-1">Architectural Identity</p>
            <p className="font-heading text-lg text-[#E8E8E8] uppercase tracking-widest">{profile.bodyType}</p>
          </div>
          <div className="text-right">
            <p className="text-[10px] uppercase tracking-widest text-[#8D8D8D] mb-1">Preferred Fit</p>
            <p className="font-heading text-lg text-[#E8E8E8] uppercase tracking-widest">{profile.preferredFit}</p>
          </div>
          <div className="text-right">
            <p className="text-[10px] uppercase tracking-widest text-[#8D8D8D] mb-1">Confidence</p>
            <p className="font-heading text-lg text-[#E8E8E8] uppercase tracking-widest">{profile.confidenceScore}%</p>
          </div>
        </div>
      </div>
    </div>
