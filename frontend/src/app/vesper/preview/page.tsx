...
export default function FitPreviewPage() {
  ...
  if (!profile) {
    return (
      <div className="h-full w-full flex flex-col items-center justify-center bg-background texture-grain p-12 text-center">
        <h2 className="font-heading text-xl uppercase tracking-[0.2em] text-[#FDFCFB] mb-4">
          No Profile Found
        </h2>
        <p className="text-sm text-[#8D8D8D] mb-8">
          You must complete the Size Intelligence analysis first.
        </p>
        <Link
          href="/vesper/size"
          className="bg-[#FDFCFB] text-[#0A0A0A] px-8 py-4 rounded-sm uppercase tracking-widest text-[11px] font-medium hover:bg-[#E8E8E8] transition-colors"
        >
          Begin Analysis
        </Link>
      </div>
  ...
  return (
    <div className="h-full w-full flex flex-col px-6 md:px-12 pt-32 pb-12 relative hide-scrollbar overflow-y-auto">
      <div className="max-w-6xl mx-auto w-full min-h-full flex flex-col justify-center">
        <header className="mb-8 flex items-center justify-between">
          ...
          <button
            onClick={() => router.push("/vesper/size")}
          ...
          <div className="w-full lg:w-80 flex flex-col shrink-0">
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center bg-[#111] p-4 border border-[#202020] rounded-sm">
                <span className="text-[10px] uppercase tracking-widest text-[#8D8D8D]">
                  Shoulders
                </span>
                <span className="text-[11px] text-[#E8E8E8]">
                  {scaleXShoulders > 1.05
                    ? "Tailored (Tight)"
                    : scaleXShoulders < 0.95
                      ? "Relaxed"
                      : "Perfect Alignment"}
                </span>
              </div>
              <div className="flex justify-between items-center bg-[#111] p-4 border border-[#202020] rounded-sm">
                ...
                <span className="text-[11px] text-[#E8E8E8]">
                  {scaleXChest > 1 ? "Form Fitting" : "Natural Drape"}
                </span>
              </div>
              <div className="flex justify-between items-center bg-[#111] p-4 border border-[#202020] rounded-sm">
                <span className="text-[10px] uppercase tracking-widest text-[#8D8D8D]">
                ...
                <span className="text-[11px] text-[#E8E8E8]">
                  {scaleXWaist > 1 ? "Structured" : "Tapered"}
                </span>
              </div>
            </div>
            ...
            <div className="mt-8 bg-[#0A0A0A] p-6 border border-[#333] relative overflow-hidden group">
              ...
              <p className="text-[9px] uppercase tracking-widest text-[#4A4A4A] mb-2 relative z-10">
                AI Recommendation
              </p>
              <h2 className="font-heading text-2xl text-[#FDFCFB] relative z-10 mb-2">
                SIZE M
              </h2>
              <p className="text-xs text-[#8D8D8D] leading-relaxed relative z-10 italic">
                &quot;Size M offers better shoulder alignment while preserving
                natural drape through the torso.{" "}
                {profile.preferredFit === "Slim"
                  ? "It provides a closer silhouette without sacrificing chest comfort."
                  : ""}
                &quot;
              </p>
            </div>
          ...
          <div className="flex-1 bg-[#0A0A0A] border border-[#202020] rounded-sm relative min-h-[600px] flex items-center justify-center overflow-hidden">
            ...
            <div className="absolute top-6 left-6 text-[9px] uppercase tracking-widest text-[#4A4A4A] flex flex-col gap-2">
              <span className="flex items-center gap-2">
                <Layers className="w-3 h-3" /> Displacement Layer Active
              </span>
              <span className="flex items-center gap-2">
                <User className="w-3 h-3" /> Profile: {profile.bodyType}
              </span>
            </div>
            ...
            <AnimatePresence mode="wait">
              {!isWarping && (
                ...
                >
                  ...
                  >
                    <path
                      d="M30,20 C40,10 60,10 70,20 L85,60 C90,80 85,120 75,180 L25,180 C15,120 10,80 15,60 Z"
                      fill="none"
                      stroke="white"
                      strokeWidth="0.5"
                    />
                  ...
                  <motion.svg
                    ...
                    animate={{
                      ...
                      scaleX: [
                        1,
                        Math.min(1.15, Math.max(0.85, scaleXShoulders)),
                      ],
                    }}
