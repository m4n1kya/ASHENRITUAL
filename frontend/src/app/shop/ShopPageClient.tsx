  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        if (selectedCategory)
          url = `/products/category/${selectedCategory}?page=${page}&limit=12`;
        if (searchQuery)
          url = `/products?q=${encodeURIComponent(searchQuery)}&page=${page}&limit=12`;
  ...
  return (
    <div className="w-full">
            ...
            <AnimatePresence initial={false}>
              <motion.img
                ...
                className={`absolute inset-0 h-full w-full object-cover object-[80%_center] md:object-center contrast-125 ${
                  SLIDESHOW_IMAGES[currentSlide].includes("monochrome-view")
                    ? "brightness-100"
                    : "brightness-50"
                }`}
              />
      ...
      <div
        ...
        ref={gridRef}
        className={cn(
          "mx-auto max-w-screen-xl px-4 md:px-8 pb-24 lg:px-12",
          searchQuery ? "pt-32 lg:pt-40" : "pt-24",
        )}
      >
        {/* Header inside grid */}
        <div className="mb-12">
          ...
          <h2 className="mt-3 font-heading text-3xl font-semibold uppercase tracking-[0.08em] text-[#E8E8E8] sm:text-4xl">
            {searchQuery ? `Results: "${searchQuery}"` : "Shop"}
          </h2>
        </div>
        ...
        {loading ? (
          ...
          <EmptyState
            icon={
              <span className="font-display text-4xl italic text-[#202020]">
                ∅
              </span>
            }
          ...
          <>
            <motion.div className="grid grid-cols-2 gap-4 sm:gap-8 md:grid-cols-2 lg:grid-cols-4 lg:gap-12">
